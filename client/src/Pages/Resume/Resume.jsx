...
const Resume = () => {
  ...
  return (
      ...
      {profileData ? (
        <>
          <div className="bg-gradient-to-r from-blue-500 to-teal-500 p-6 text-white text-center">
            <div className="mb-3">
              <img className="h-24 w-24 rounded-full mx-auto" src={`${import.meta.env.VITE_API_URL}/${profileData.profilePicture}`} alt="Profile picture" />
            </div>
            ...
            <p>{profileData.martialstatus} | {profileData.religion}</p>
    ...
    <div class="bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <ul>
            <li><strong>Father's Name:</strong> {profileData.fathername}</li>
            <li><strong>Nationality:</strong> {profileData.nationality}</li>
            <li><strong>CNIC:</strong> {profileData.CNIC}</li>
            <li><strong>CNIC Expiry:</strong> {profileData.CNICexpiry}</li>
        </ul>
    </div>
    ...
    <div class="bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <ul>
            <li><strong>Land Line:</strong> {profileData.landline}</li>
            <li><strong>Postal Address:</strong> {profileData.postaladdress}</li>
            <li><strong>Residential Country:</strong> {profileData.ResCountry}</li>
            <li><strong>Residential City:</strong> {profileData.ResCity}</li>
        </ul>
    </div>
    ...
    <div class="bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <ul>
            <li><strong>Hafiz Quran:</strong> {profileData.hafizquran}</li>
            <li><strong>Ex-Service Official:</strong> {profileData.ExServiceOfficial}</li>
            <li><strong>Government Official:</strong> {profileData.Governmentofficial}</li>
            <li><strong>Disabled:</strong> {profileData.Disabled}</li>
        </ul>
    </div>
    ...
    <div class="bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <ul>
            <li><strong>Job Title:</strong> {profileData.JobTitle}</li>
            <li><strong>Career Level:</strong> {profileData.CareerLevel}</li>
            <li><strong>Target Monthly Salary (PKR):</strong> {profileData.TargetMonthlySalary}</li>
            <li><strong>Last Monthly Salary (PKR):</strong> {profileData.LastMonthlySalary}</li>
        </ul>
    </div>
</div>
...
<div className="p-6 bg-white">
    <div className="mb-6">
        <h2 className="text-xl font-semibold mb-4">Professional Experience(s)</h2>
                    ...
                    {profileData.jobs.map((job, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                            ...
                            <td className="px-4 py-2">
                                {new Date(job.from).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} - 
                                {new Date(job.to).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                            </td>
...
<div className="p-6 bg-white">
    <h2 className="text-xl font-semibold mb-4">Academic Information(s)</h2>
        ...
        <table className="w-full">
            ...
            <tbody>
                {profileData.academics.map((academic, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                        <td className="px-4 py-2">{academic.degreeLevel} - {academic.degree}</td>
                        <td className="px-4 py-2">{academic.institute}</td>
                        <td className="px-4 py-2">
                            {new Date(academic.startdate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} - 
                            {new Date(academic.completiondate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                        </td>
...
<div className="p-6 bg-white">
    ...
    {profileData.certification && profileData.certification.length > 0 ? (
        <table className="w-full">
            <thead>
                <tr className="bg-gray-200">
        ...
        <table className="w-full">
            <thead>
                <tr className="bg-gray-200">
                    ...
                    <th className="px-4 py-2 text-left">Achievement Descriptions</th>
            ...
            <tbody>
                {profileData.achievements.map((trainings, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                        <td className="px-4 py-2">{trainings.AchievementTitle}</td>
                        <td className="px-4 py-2">{trainings.AchievementDescriptions}</td>
                    </tr>
                ))}
            </tbody>
        </table>
            ...
            <tbody>
                {profileData.research.map((trainings, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                        ...
                        <td className="px-4 py-2">{trainings.PublicationVenue}</td>
...
<button id="downloadButton" onClick={downloadPDF} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
    Download CV
  </button>
</div>
