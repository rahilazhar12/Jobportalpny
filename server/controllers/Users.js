...
//         const pnyalumini = await PNYAlumniSchema.findOne({ email });
...
const UserLogin = async (req, res) => {
    try {
        ...
        const token = JWT.sign(
            { id: currentUser._id, role: currentUser.role, name: currentUser.name },
...
const Profileregister = async (req, res) => {
    try {
        ...
        const userId = req.params.id; // Extracted from URL parameter
        const userName = req.user.name; // Extracted from JWT token or another authentication mechanism

        // Check if a profile with the same userId already exists
        const existingUser = await profilebuilderschema.findOne({ userId });
        if (existingUser) {
